...
在项目根目录**双击** **`startup.bat`**，脚本会自动完成以下 9 个步骤：

| 步骤     | 自动完成的内容                                           |
| ------ | ------------------------------------------------- |
| \[1/9] | 检查 Python 环境                                      |
| \[2/9] | 创建虚拟环境 `venv`（如已存在则跳过）                            |
| \[3/9] | 安装后端 Python 依赖（`pip install -r requirements.txt`） |
| \[4/9] | 检查 Node.js 环境                                     |
| \[5/9] | 安装前端依赖（`npm install`，如已存在则跳过）                     |
| \[6/9] | 检查 Ollama 服务                                      |
| \[7/9] | 检查并自动拉取模型 `qwen3:8b`（如已存在则跳过）                     |
| \[8/9] | 测试模型连通性                                           |
| \[9/9] | 检查数据库：有数据则跳过，不可连接则自动从 `OpsCenter.bak` 还原          |
